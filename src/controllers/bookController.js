...
const getBookById = async function (req, res) 
{
    try 
    {
      ...
      const reviewsData = await reviewModel.find({ bookId: bookId, isDeleted: false },{ _id: 1, bookId: 1, reviewedBy: 1, rating:1, releasedAt: 1 });
        
      return res.status(200).send({ status: true, message: 'Book List :-', data: {...bookDetail.toObject(),reviewsData,reviews : reviewsData.length}});
    } 
    catch (error) 
    {
